...
```
净资产 = 现金余额 + 存款(active) + 持仓市值 + 借出款 - 负债
```
...
**问题**：到期存款仍显示在首页资产中，银行卡片上显示"已到期 N"但点进去看不到到期存单。

**修复**：
- 到期处理独立于 backfill，每次 App 启动都检查并处理到期存款
- 银行组卡片点击 → 进入银行详情页，同时展示活跃和已到期存单（分区显示）
- 存款页底部新增「已到期存单」汇总入口，点击查看全部已到期存单（按银行分组）
...
**设计原则：持仓总额 > 当前价格 ≈ 当日收益，其余弱化。美股同步显示美元原价。**

#### GroupCard（分组卡片）

```
┌──────────────────────────────────────────────┐
│ ▌ AAPL  US               Apple Inc.          │  symbol + 市场芯片 + 名称
│ ▌                                           │
│ ▌  持仓市值  ¥128,500.00                     │  L0: 22sp Bold
│ ▌                                           │
│ ▌  现价 ¥2,570.00       今日 +¥2,350         │  L1: 现价 18sp Bold / 今日 16sp SemiBold
│ ▌  ≈$352.80 USD               +1.86%        │  仅美股: 14sp 美元价 + 涨幅%
│ ▌                                           │
│ ── ─── 迷你走势图 ──────────────────────     │
│ ▌                                           │
│ ▌  持仓盈亏 +¥12,500 (+10.8%)                │  L2: 13sp
│ ▌  持有 50股 · 成本均价 ¥2,164.00           │  L2: 12sp
└──────────────────────────────────────────────┘
```
...
│ ▌ 腾讯控股                      持有 100股    │
│ ▌ 00700                                      │
│ ▌  市值 ¥38,520.00                           │  L0: 20sp Bold
│ ▌  现价 ¥385.20          今日 +¥520          │  L1: 16sp Bold / 14sp SemiBold
│ ▌  成本 ¥320.00    持仓盈亏 +¥6,520          │  L2: 12sp
└──────────────────────────────────────────────┘
```
...
│ ← 贵州茅台 (600519)                           │
│  现价                                        │
│  ¥1,720.00                    +1.18%         │  L1: 32sp ExtraBold / 涨幅 20sp
│  ≈$xxx USD (仅美股)            +¥20.00       │  仅美股
│  市值 ¥172,000.00           今日 +¥200       │  L0: 18sp Bold / 今日 16sp
│  [走势图区域]                                 │
│  持仓盈亏 +¥4,000 (+2.38%)                    │  L2
│  持有 100股 · 成本均价 ¥1,680 · 总成本 ¥168,000│  L2
│ [ + 加仓 ]        [ - 减仓 ]                 │
└──────────────────────────────────────────────┘
```
...
- `HoldingDisplay`、`HoldingGroupDisplay`、`DetailUiState` 各新增 `originalPrice: String` 字段
- 仅 `market == "US"` 时显示 `≈{originalPrice} USD`
- ViewModel 在构造 Display Model 时保留原始货币价格（不经过汇率换算）

### 黄金卡片（v23 重设计）

黄金是单一资产、多次买入，所有买入归入**一张聚合概览卡片**。点击进入独立详情页。**v24 新增减仓和删除全部持仓功能。**
...
- 点击卡片 → 进入 `GoldDetailScreen`（独立详情页）
- FAB → 弹出买入对话框
- **数据模型**：1 个 Holding（汇总） + N 个 Transaction（买入/卖出记录）
- Holding.quantity = 总克数，Holding.costPrice = 加权均价
- Transaction.type = "BUY" 或 "SELL"
- 新增/编辑/删除买入时重算 Holding

#### 内层：黄金详情页（GoldDetailScreen）

独立详情页，包含完整走势图、买入记录列表、加减仓操作、删除全部持仓。

- TopAppBar：返回 + "黄金 (XAU)" + 右侧删除图标
- 市值 Hero 区：大号市值 + 今日涨跌
- 走势图：30 天价格折线图 + 买入点标注 + 1月/3月/1年切换
- 盈亏双卡 + 持仓信息（持仓量/均价/总成本）
- **加仓/减仓按钮**：黄金也支持加减仓（去掉之前的限制）
- 买入记录列表：每笔可编辑/删除
- **删除全部持仓**：底部红色按钮 + 二次确认弹窗

#### 减仓弹窗（新增）

- 卖出克数 + 卖出单价 + 卖出日期
- 实时预览：剩余克数、实现盈亏、剩余成本均价
- ☐ 收入计入现金
- 减仓克数 = 持仓克数 → 全部清仓 → 二次确认

#### 删除全部持仓

- 二次确认："删除全部黄金持仓后将无法恢复，确定删除吗？"
- 确认后删除 Holding + 所有 Transaction + 所有 PriceSnapshot
- 返回 GoldPage 空状态
...
| 单价 | 每克价格（元/克） |
| 克数 | 黄金重量（克） |
| 从现金扣除 | 可选，勾选后自动生成出金流水 |

> **v23 变更**：黄金改为加仓模式。每次买入创建一条 Transaction（type=BUY），自动更新 Holding 汇总（quantity、costPrice 重算）。买入总价 = 单价 × 克数，自动计算展示。
...
按月份分组展示每周收益卡片（2 列网格）。每月显示 4-5 周，每周卡片显示日期范围 + 周收益汇总。点击某周 → 下方展示该周各资产明细。未来周（还没到的周）显示"—"，虚线边框 + 灰底半透明，不可点击。详见 UI_DESIGN.md Section 8.2。
...
顶部 pill 切换「收益」/「总资产」两种模式：

- **收益模式**（默认）：年度卡片列表，显示每年总收益 + 涨/跌/活跃天数。数据来自 `DailyBreakdownItem.changeCNY` 聚合。
- **总资产模式**：年度卡片列表，显示每年末总资产 + 较上年变化额 + 变化率。数据来自 `DailySummary.totalValueCNY` 年末快照。

切换不重新加载数据，两套数据在进入年视图时一次性加载。资产增额 = 收益 + 净存入，总资产模式让用户看到完整资产增长全貌。

详见 UI_DESIGN.md Section 8.4。
...
data class CashTransaction(
    ...
    val type: String,                // MANUAL / DEPOSIT_MATURITY / LEND / REPAY
    val note: String = "",
    val relatedId: Long? = null      // 关联的 Receivable/Debt id
)
```
...
|------|------|
| `MANUAL` | 手动入金/出金 |
| `DEPOSIT_MATURITY` | 存款到期自动入账 |
| `LEND` | 借钱给别人（出金），新增借出时勾选"现金同步扣减" |
| `REPAY` | 别人还款（入金），标记借出"对方已还款"时自动生成 |
...
## 二十五、资产增涨爽感系统（v21 — 2026-05-24）

### 25.0 设计目标

让用户每次打开 App 都能感知到资产在增长，从而激励持续存钱。所有设计遵循克制、精致的 Apple 风格，不落入游戏化俗套。

### 25.1 核心功能

| 功能 | 说明 |
|------|------|
| 首页数字滚动 | 进入首页时总资产执行 count-up 动画（800ms），结束时金色光晕闪现 |
| 正收益金色脉冲 | 今日收益 > 0 时，数字周围淡金色呼吸光晕 |
| 连续增长标识 | 连续 N 天正收益 → 显示"连涨 N 天"金色徽章 |
| 环比数据 | 总资产下方展示"较上月"变化，让中长期进步可视化 |
| 里程碑动效 | 突破 10万/50万/100万 时金色粒子微庆祝 |
| 存款利息呼吸 | 存款卡片"今日利息"微弱闪烁，暗示"钱在生钱" |
| 存入正反馈 | 存钱完成后直接展示新总资产，而非仅"保存成功" |
| 空状态引导 | 新用户看到温和引导页，引导添加第一笔资产 |

### 25.2 数据依赖

- 连续增长标识：`DailySummary` 表 `dayChange` 字段，往前数连续正值天数
- 环比数据：`DailySummary` 表上月同日记录
- 里程碑触发：SharedPreferences 记录已触发的里程碑档位

### 25.3 设计约束

- 所有动效检查 `prefers-reduced-motion`，开启时降级为静态显示
- 下跌时安静显示，不加焦虑感（不弹窗、不标红强调）
- 金色仅用于正面激励，不用于警告或错误状态

---

## 二十六、持仓分组重设计（v17 — 2026-05-23）
